...
t_redirection_set	*chunk_add_back(t_redirection_set **chunks, char **argv)
{
	t_redirection_set	*new;
	size_t	index;

	new = malloc(sizeof(t_redirection_set));
	if (!new)
		exit_fatal();
	...
	new->type = get_redirection_type(argv[new->size]);
	new->next = NULL;
	new->prev = NULL;	
